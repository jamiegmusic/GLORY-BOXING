...
const mockMatchData = {
  id: 'test-match-1',
  fighter_a: 'Carl Froch',
  fighter_b: 'Tony Bellew',
...
describe('CommentaryPanel', () => {
  ...
  test('displays match information correctly', () => {
    render(
      <CommentaryPanel 
        matchData={mockMatchData}
        commentary={mockCommentaryData.commentary}
    ...
    expect(screen.getByText('O2 Arena, London')).toBeInTheDocument()
    expect(screen.getByText('Light Heavyweight')).toBeInTheDocument()
  })

  test('handles empty match data gracefully', () => {
    ...
    render(<CommentaryPanel matchData={emptyMatchData} />)
    
    expect(screen.getByText('Fight Commentary')).toBeInTheDocument()
    expect(screen.getByText('Generate AI Commentary')).toBeInTheDocument()
  })

  test('toggles commentary sections correctly', () => {
    render(
      <CommentaryPanel 
        matchData={mockMatchData}
        commentary={mockCommentaryData.commentary}
    ...
    const highlightToggle = screen.getByText('Key Highlights')
    fireEvent.click(highlightToggle)

    // Section should be collapsed
    expect(screen.queryByText('Carl Froch wins by ko')).not.toBeInTheDocument()
  })
}) 