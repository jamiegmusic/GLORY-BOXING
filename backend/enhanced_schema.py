...
class Mutation:
    @strawberry.mutation
    def schedule_match(self, fighter_a_id: int, fighter_b_id: int, venue: str, date: str) -> Match:
        fighter_a = next(f for f in fighters if f.id == fighter_a_id)
        fighter_b = next(f for f in fighters if f.id == fighter_b_id)
        match = Match(id=len(matches)+1, fighter_a=fighter_a, fighter_b=fighter_b, venue=venue, date=date)
