import type { Fighter, Injury, HealthMonitoring } from './unified-types'
...
export class HealthMonitoringSystem {
  ...
  public simulateInjury(fighter: Fighter, injuryType: string, severity: 'mild' | 'moderate' | 'severe'): Injury {
    ...
    const injury: Injury = {
      ...
      recovery_date: new Date(Date.now() + recoveryTime * 24 * 60 * 60 * 1000),
      impact_on_performance: injuryAssessment.impactOnPerformance,
      treatment_plan: injuryAssessment.treatmentRequired,
      description: `Simulated ${severity} ${injuryType} injury`,
      active: true
    }
