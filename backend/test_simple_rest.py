#!/usr/bin/env python3
"""
Test script for simple REST API backend
"""
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            response = await client.post(
                "http://localhost:8000/api/fighters",
                json=fighter_data,
                headers=headers
            )
            if response.status_code == 200:
        ...
        try:
            response = await client.get("http://localhost:8000/api/matches")
            if response.status_code == 200:
                data = response.json()
...
async def main():
    ...
    print("\n" + "=" * 50)
    print("✅ Simple REST API test completed!")
    print("\n📚 Available endpoints:")
